...
from typing import Optional  # 👈 Importamos Optional para que funcione en Python 3.9
...
async def response_json(item_id: int, q: Optional[str] = None):  # 👈 Cambiado str | None por Optional[str]
    return JSONResponse(
...
async def response_html(item_id: int, item_name: Optional[str] = None):  # 👈 Cambiado str | None por Optional[str]
    html_str = f"""
    <html>
    <body>
        <h2>HTML Response</h2>
...
@app.get("/redirect")
async def redirect_only(comment: Optional[str] = None):  # 👈 Cambiado str | None por Optional[str]
    print(f"redirect: {comment}")
    return RedirectResponse(url=f"/resp-html/3?item_name={comment}")


# ── 3-2. POST → GET 리da이렉트 (302 필수) ────────────────
...
class Item(BaseModel):
    ...
    description: str
    price: float
    tax: Optional[float] = None    # 👈 Cambiado float | None por Optional[float]
